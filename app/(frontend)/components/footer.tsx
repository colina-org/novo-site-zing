...
export default async function Footer() {
  const data = await getGlobal("footer");
  const socials: { network?: string; url?: string }[] = data?.socials ?? [];
  const findSocial = (n: string) =>
    socials.find((s) => s.network === n)?.url;
  const logoZing = mediaUrl(data?.logoZing) || "/logo-header.svg";
  const logoIebt = mediaUrl(data?.logoIebt) || "/iebt-logo.png";
          ...
          <div className="flex flex-col items-start justify-between gap-10 md:flex-row md:items-center">
            ...
            <div className="flex flex-col gap-10">
              ...
              <div className="flex items-center gap-3">
                ...
                {findSocial("instagram") && (
                  <SocialButton
                    href={findSocial("instagram")!}
                    target="_blank"
                    label="Instagram"
                  >
                    <InstagramIcon />

...
type HeaderProps = {
  ...
  sectorItems?: NavLabel[];
  contentItems?: NavLabel[];
  navItems?: NavLink[];
  loginCta?: NavLink;
  demoCta?: NavLink;
  assistantCta?: NavLink;
};
    ...
    <header className="sticky top-0 z-50 w-full bg-white">
      <div className="container mx-auto flex h-20 items-center justify-between px-6">
        ...
        <Link href="/" aria-label="Zing - Página inicial" onClick={() => setMenuOpen(false)}>
          <Image src={logo || "/logo-header.svg"} alt="Zing" width={112} height={50} priority />
        </Link>
        ...
        <div className="hidden items-center gap-4 nav:flex">
          ...
          <Link href={demoCta?.href ?? "/contato"} className="btn-primary">
            {demoCta?.label}
          </Link>
        </div>

        <div className="flex items-center gap-4 nav:hidden">
          <Link href={assistantCta?.href ?? "#"} target="_blank" className="group">
            <Image

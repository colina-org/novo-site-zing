...
type HeroContatoProps = {
  title?: unknown;
  description?: unknown;
  cargoOptions?: Option[];
  servicoOptions?: Option[];
  contatoOptions?: Option[];
};
...
export default function HeroContato({
  ...
  function toggleContato(method: string) {
    setContatos((prev) =>
            ...
            <div className="flex flex-wrap gap-6">
              {contatoOptions.map((opt) => (
                ...
                >
                  <input
                    type="checkbox"
                    checked={contatos.includes(opt.value ?? "")}
                    onChange={() => toggleContato(opt.value ?? "")}
